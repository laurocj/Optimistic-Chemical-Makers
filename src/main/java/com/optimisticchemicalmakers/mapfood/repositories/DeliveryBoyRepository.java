...
public interface DeliveryBoyRepository extends CrudRepository<DeliveryBoy, Long> {
    ...
    @Query(value="SELECT d.*, "+HAVERSINE_PART+" AS distance FROM deliveryBoy d HAVING distance <= :distance order by distance",nativeQuery = true)
    List<DeliveryBoy> getNearestDeliveryBoys(@Param("latitude") Double latitude, @Param("longitude") Double longitude, @Param("distance") Double distance);
